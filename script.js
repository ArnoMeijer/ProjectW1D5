...
const guess = function () {
  ...
  if (wordToDisplay.join('') == wordToGuess.join('')) {
    livesNode.innerHTML = `U hebt gewonnen! gefeliciteerd`;
...
const restart = function () {
  guessBtnNode.disabled = false;
  letterNode.value = '';
  wordToGuess = wordArray[Math.floor(Math.random() * (wordArray.length + 1))].split('');
  wordToDisplay = wordToGuess.map(e => '_ ');
  lettersGuessed = [];
  lives = 5;
  wordNode.innerHTML = wordToDisplay.join('');
...
document.addEventListener('DOMContentLoaded', () => {
  letterNode = document.getElementById('letter');
